...
export async function generateMetadata({ params: { lang } }: { params: { lang: Locale } }): Promise<Metadata> {
  // fetch data
  const metadata = await getLocalizedData<MetaDataType>(lang, 'projects.threeBedroomApartmentAv.metadata');
  return {
    ...metadata,
  };
}
...
const page = async ({ params: { lang } }: { params: { lang: Locale } }) => {
  const { banner, header, swipers } = await getLocalizedData<AvelineInnerPagesType>(lang, 'projects.threeBedroomApartmentAv');
  return (
    <>
      ...
      <Suspense>
        {
          swipers?.map((swiper, index) => <SwiperSection key={index + "-swiper3"} animateFrom={index % 2 === 0 ? 'left' : 'right'} imagesArray={swiper.arrayMapName} title={swiper.title} className='section-py' />)
        }
      </Suspense>
