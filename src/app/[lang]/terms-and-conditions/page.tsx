...
export async function generateMetadata({ params: { lang } }: { params: { lang: Locale } }): Promise<Metadata> {
  // fetch data
  const metadata = await getLocalizedData<MetaDataType>(lang, 'termsAndConditions.metadata');
  return {
    ...metadata,
  };
}
const TermsAndConditionsPage = async ({ params: { lang } }: { params: { lang: Locale } }) => {
  const { title, htmlText } = await getLocalizedData<PrivacyPolicyAndTermsConditionsType>(lang, 'termsAndConditions');
  return (
    <>
